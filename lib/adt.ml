...
let rec insert new_value = function
  ...
  | Node (left, value, right) when new_value > value ->
      Node (left, value, insert new_value right)
  | Node (left, value, right) when new_value < value ->
      Node (insert new_value left, value, right)
  | x -> x

let%test "summation" =
  sum_tree (Node (Node (Leaf, 3, Leaf), 5, Node (Leaf, 7, Leaf))) = 15
...
let%test "insertion empty tree" = insert 3 Leaf = Node (Leaf, 3, Leaf)
